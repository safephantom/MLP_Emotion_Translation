...
## 4. 주요 실험 결과 요약
실험은 아래 두 가지 번역 조건을 대조군과 실험군으로 설정하여 다차원 평가를 진행했습니다.
1. **대조군(Baseline)**: 감정 정보가 제공되지 않은 단순 텍스트 번역.
2. **실험군(Proposed)**: 음향 모델 및 종결어미(EF) 가중치 모델을 통해 예측된 VAE 정보가 프롬프트로 제공된 번역.
...
## 참여 기여자
본 프로젝트는 대학원 과정 내 두 명의 연구진의 협업으로 이루어졌습니다.

* **감정 인식 모델링** ([modeling/](file:///c:/Project/MLP_Emotion_Translation/modeling)): KEMDy19 데이터셋 가공, Soft Labeling 도입, Multimodal LSTM 모델 및 Optuna 파라미터 튜닝 구현.
* **번역 및 평가 파이프라인** ([translation_eval/](file:///c:/Project/MLP_Emotion_Translation/translation_eval)): 감정 주입형 프롬프트 엔지니어링 설계, VAD-BERT 기반 정서 보존성 검증, LLM Judge 및 Human Evaluation 실험 수행 및 통계 분석.
