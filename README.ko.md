...
* **실험군(멀티모달: 음향 + 종결어미)**: 정확도(Hard Accuracy) **59.17%** 달성 (Valence 손실: 0.0501, Arousal 손실: 0.0168).
* **대조군(음향 단독 baseline)**: 정확도가 **44.28%**로 급감 (Valence 손실: 0.0660, Arousal 손실: 0.0203).
* **핵심 인사이트**: 문법적 종결어미 가중치를 제외하는 경우 감정 분류 정확도가 **약 15%p 하락**하며, 종결어미가 한국어 구어체의 정서 파악에 있어 핵심적인 맥락 해결 장치임을 보여줍니다.
